      ...
      <Table>
        ...
        <TableHeader className="sticky top-0 bg-muted/50 backdrop-blur-sm">
          <TableRow>
            ...
            <TableHead className="text-right text-green-500">Returns (L)</TableHead>
            <TableHead className="text-right text-red-500">Expenses (L)</TableHead>
        ...
        <TableBody>
          {data.map((entry) => (
            <TableRow key={entry.year} className={cn(entry.closingBalance <= 0 && "bg-destructive/10")}>
              ...
              <TableCell className="text-right text-green-600 dark:text-green-500">{formatCurrency(entry.investmentReturns)}</TableCell>
